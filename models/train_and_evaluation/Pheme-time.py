...
class DPSG(nn.Module):
    def __init__(self, input_dim, n_hidden_dim, u_hidden_dim, p_hidden_dim, out_embed_d, outemb_d=1,
        ...
        #新加内容
        # ===== Ablation: -MDF (concat + linear fusion) =====
        self.mdf_linear = nn.Linear(
            2 * self.hidden_dim,   # graph_feature + content_feature
            self.hidden_dim
    ...
    def transformer_agg(self, het_node, neighbor_order_n_p_u, neighbor_order_n, all_t0, npu=15, n=5):
        ...
        #新加内容
        # ===== Ablation: -MDF simple fusion =====
        fusion = torch.cat([align_c, align_g], dim=1)   # [B, 2H]
        fusion = self.mdf_linear(fusion)                # [B, H]
        ...
        final_representation = fusion.view(fusion.size(0), 1, fusion.size(1))
        return final_representation, dist
...
base_PATH = '/home/zhangrq/DPSG/models/ablation/pheme_minus_MDF/'
