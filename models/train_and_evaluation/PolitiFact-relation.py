...
class DPSG(nn.Module):
    ...
    def transformer_agg(self, het_node, neighbor_order_n_p_u, neighbor_order_n,all_t0, npu=200,n=5):
        ...
        # ===== -EGSE: 用 LSTM 替代关系增强 =====
        lstm_input = node_embedding_n.unsqueeze(0)   # [1, seq_len, hidden_dim]
        lstm_out, (h_n, c_n) = self.encoder_lstm(lstm_input)
        final_representation = h_n[-1].unsqueeze(0).unsqueeze(0)
        # ===== -EGSE: 直接返回，后续 Transformer / GAT 全部不走 =====
        dist = [node_embedding_n.mean(dim=0), node_embedding_n.mean(dim=0)]
...
base_PATH = '/home/zhangrq/DPSG/models/ablation/politifact_minus_RE/'
if not os.path.exists(base_PATH):
    os.makedirs(base_PATH)
f_path = base_PATH + 'result-relation.txt'
