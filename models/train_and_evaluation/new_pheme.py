...
def data_loader(pathway='/home/zhangrq/DPSG/data/processed_data/PHEME/pheme_n5_p5_u100/normalized_news_nodes/', node_type="news"):
    # 定义公共变量
    nodes = []
    file_list = sorted([f for f in os.listdir(pathway) if f.startswith("batch_") and f.endswith(".txt")])

    if not file_list:
        print("No batch files found in the specified pathway.")
        return nodes

    # 根据节点类型定义变量
    if node_type == "news":
        id_list, label_list, embed_list = [], [], []
        n_neigh_list, p_neigh_list, u_neigh_list = [], [], []
    elif node_type == "post":
        id_list, embed_list = [], []
    elif node_type == "user":
        id_list, embed_list = [], []
    ...
    for file_name in file_list:
        file_path = os.path.join(pathway, file_name)
        ...
        for i, line in enumerate(lines):
            if node_type == "news":
                if i % 5 == 0:
                    _, id_, label = line.split()
                    id_list.append(id_)
                    label_list.append(int(label))
                    current_embed = []
                elif i % 5 == 1:
                    current_embed.append(list(map(float, line.split())))
                    embed_list.append(current_embed)
                elif i % 5 == 2:
                    n_neigh_list.append(line.split())
                elif i % 5 == 3:
                    p_neigh_list.append(line.split())
                elif i % 5 == 4:
                    u_neigh_list.append(line.split())
    ...
    for i, id_ in enumerate(id_list):
        if node_type == "news":
            node = Het_Node(
                node_type="news", node_id=id_, embed=embed_list[i],
                neighbor_list_news=n_neigh_list[i], neighbor_list_post=p_neigh_list[i],
                neighbor_list_user=u_neigh_list[i], label=label_list[i]
            )
        elif node_type == "post":
            node = Het_Node(node_type="post", node_id=id_, embed=embed_list[i])
        elif node_type == "user":
            node = Het_Node(node_type="user", node_id=id_, embed=embed_list[i])
        nodes.append(node)
...
class DPSG(nn.Module):
    def __init__(self, input_dim, n_hidden_dim, u_hidden_dim, p_hidden_dim, out_embed_d, outemb_d=1,
        ...
        self.init_linear_feature = nn.Linear(112, self.hidden_dim)
    ...
    def Bi_RNN(self, neighbor_id, neighbor_time,node_type, post_emb_dict, user_emb_dict, news_emb_dict):
        ...
        if node_type == "user":
            ...
            input_b = []
            for i in neighbor_id:
                if i not in user_emb_dict:  # 如果用户嵌入不存在，则跳过
                    continue
                user_embedding = user_emb_dict[i]
                input_a.append(user_embedding[0])  # 取第一个元素
                input_b.append(user_embedding[1])  # 取第二个元素

            # 检查 input_a 是否为空，如果为空则用默认值填充
            if len(input_a) == 0:
                input_a.append(torch.zeros(self.hidden_dim))  # 匹配 hidden_dim
                input_b.append(torch.zeros(self.hidden_dim))  # 匹配 hidden_dim

            try:
                input_a = torch.stack([torch.Tensor(a) for a in input_a]).to(device)
                input_b = torch.stack([torch.Tensor(b) for b in input_b]).to(device)
            except Exception as e:
                print(f"Error converting input_a or input_b to tensors: {e}")
                raise
            ...
            # 确保形状正确
            linear_input_other = linear_input_other.view(linear_input_other.shape[0], 1, self.hidden_dim)
            linear_input_content = linear_input_content.view(linear_input_content.shape[0], 1, self.hidden_dim)

            # 注意力机制
            attention_out_other, self.hidden_other = self.attention_other_user(
                linear_input_other, linear_input_other, linear_input_other
            )
            attention_out_content, self.hidden_text = self.user_content_attention_text(
                linear_input_content, linear_input_content, linear_input_content
            )
...
five_res = []
base_PATH = '/home/zhangrq/DPSG/models/pre-trained/pheme/'
#base_PATH = '../pre-trained/pheme/'
