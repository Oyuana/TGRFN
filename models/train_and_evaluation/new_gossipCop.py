...
def neighbor_loader(pathway):
    ...
    with open(pathway) as f:
        for line in f:
            ...
            key, neighbor_str = line.split(':', 1)
            neighbors = neighbor_str.strip().split()
            ...
            for item in neighbors:
                ...
                neighbor_dict_n_p_u_add_time.setdefault(key, []).append(time_value)

            neighbor_dict_n_p_u[key] = [(item[0], item[1:]) for item in neighbors if item[1:] != 'PADDING']
            neighbor_dict_n[key] = [(item[0], item[1:]) for item in neighbors if item[0] == 'n' and item[1:] != 'PADDING']
            neighbor_dict_news[key] = [item[1:] for item in neighbors if item[0] == 'n' and item[1:] != 'PADDING']
            neighbor_dict_user[key] = [item[1:] for item in neighbors if item[0] == 'u' and item[1:] != 'PADDING']
            neighbor_dict_post[key] = [item[1:] for item in neighbors if item[0] == 'p' and item[1:] != 'PADDING']
...
neighbor_dict_n_p_u, neighbor_dict_n, neighbor_dict_n_p_u_add_time, neighbor_dict_news, neighbor_dict_post, neighbor_dict_user = neighbor_loader('/home/zhangrq/DPSG/data/rwr_results/fnn_gossipcop_u50/n_neighbors.txt')
...
class DPSG(nn.Module):
    ...
    def transformer_agg(self, het_node, neighbor_order_n_p_u,all_t0,npu=30):
        ...
        for i in all_t0:
            try:
        # 尝试从字符串中直接转换为 float，无论是否带 t
                if isinstance(i, str):
                    if i.startswith('t'):
                        all_t.append(float(i[1:]))
                    else:
                        all_t.append(float(i))
                elif isinstance(i, (float, int)):
                    all_t.append(float(i))
                else:
                    print(f"[WARN] Skipped non-time value: {i}")
            except Exception:
                print(f"[WARN] Skipped non-time value: {i}")

        if not isinstance(all_t, list) or len(all_t) < 2:
            print(f"[ERROR] Skipping due to invalid all_t: {all_t0} -> parsed: {all_t}")
            dummy_tensor = torch.zeros(1, 1, self.hidden_dim).to(device)
...
five_res = []
base_PATH ='/home/zhangrq/DPSG/models/pre-trained/GossipCop/'
...
for repeat in range(5):
        ...
        for epoch in range(start_epoch,num_epoch):
            ...
            for i in range(len(train_set)):
                ...
                if i % batch_size == 0:
                    ...
                    loss_dis = torch.tensor(0.0, requires_grad=True).to(device)
                ...
                key = 'n' + node_id
                if key not in neighbor_dict_news:
                    print(f"Missing node_id in neighbor_dict_news: {key}")
                    continue
                all_t.extend(neighbor_dict_news[key])
                if len(all_t) <= 1:
                    print(f"Insufficient time data for node_id: {node_id}")
                    continue

                neighbors = neighbor_dict_n_p_u.get(node_id, [])
                output, dist_og = net(train_set[i], neighbors, all_t)
                out = output.item()
            ...
            for j in range(len(val_set)):
                ...
                neighbors = neighbor_dict_n_p_u.get(node_id, [])
                output, dist_og = net(val_set[j], neighbors, all_t)
                out = output.item()
            ...
            denom = real_true + fake_count - fake_true
            if denom == 0:
                real_precision = 0.0  # 或者 float('nan')
            else:
                real_precision = real_true / denom
    ...
    for k in range(len(test_set)):
        ...
        for fold in range(num_folds):
            ...
            all_t.append(n_add_time_dict[node_id])
            all_t.extend(neighbor_dict_news[key])
            if len(all_t) <= 1:  # 只有自身时间或为空
                (f"Insufficient time data for node_id: {node_id}")
                continue
            neighbors = neighbor_dict_n_p_u.get(node_id, [])
            result, dist_og = best_models[fold](test_set[k], neighbors, all_t)

            output += result.item()
            tloss = BCELoss(predictions=result, true_label=test_set[k].label)
            tloss_dis = nn.MSELoss()(dist_og[0], dist_og[1])
