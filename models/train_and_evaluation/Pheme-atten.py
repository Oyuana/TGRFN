...
class DPSG(nn.Module):
    ...
    def transformer_agg(self, het_node, neighbor_order_n_p_u, neighbor_order_n, all_t0, npu=15, n=5):
        ...
        # ===== -NSP: use LSTM instead of Transformer =====

        # encoder_emb_input 原本是邻居序列，我们复用其“语义来源”
        # 这里直接使用 align_c_all 之前的 encoder_emb_input 语义来源
        # 但为了简单、稳定，使用 node_embedding_n 构造序列

        # node_embedding_n: [num_news_nodes, hidden_dim]
        lstm_input = node_embedding_n.unsqueeze(0)  # [1, seq_len, hidden_dim]

        lstm_out, (h_n, c_n) = self.nsp_lstm(lstm_input)
...
five_res = []
base_PATH = '/home/zhangrq/DPSG/models/ablation/pheme_CA/'
if not os.path.exists(base_PATH):
    os.makedirs(base_PATH)
f_path = base_PATH + 'result-CrossAttention.txt'
