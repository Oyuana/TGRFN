...
p_add_time_dict={}
n_add_time_dict={}
with open("/home/zhangrq/DPSG/data/rwr_results/fnn_gossipcop_u50/n_add_time.txt",'r') as f:
    for line in f:
        line = line.strip()
        if line:
            key, value = line.split(' ', 1)
            n_add_time_dict['gossipcop-'+key] = value
with open("/home/zhangrq/DPSG/data/rwr_results/fnn_gossipcop_u50/p_add_time.txt",'r') as f:
    for line in f:
        line = line.strip()
        if line:
            key, value = line.split(' ', 1)
...
neighbor_dict = neighbor_loader('/home/zhangrq/DPSG/data/rwr_results/fnn_gossipcop_u50/n_neighbors.txt')

with open("/home/zhangrq/DPSG/data/rwr_results/fnn_gossipcop_u50/original_adj",'r') as f:
...
def data_loader(pathway,node_type):
    if node_type == "news":
        ...
        return user_node
post_nodes = data_loader(pathway='/home/zhangrq/DPSG/data/processed_data/FakeNewsNet/GossipCop/fnn_gossipcop_50/normalized_post_nodes/', node_type="post")
user_nodes = data_loader(pathway='/home/zhangrq/DPSG/data/processed_data/FakeNewsNet/GossipCop/fnn_gossipcop_50/normalized_user_nodes/', node_type="user")
news_nodes = data_loader(pathway='/home/zhangrq/DPSG//data/processed_data/FakeNewsNet/GossipCop/fnn_gossipcop_50/normalized_news_nodes/', node_type="news")
...
class DPSG(nn.Module):
    ...
    def Bi_RNN(self, neighbor_id, neighbor_time, node_type,
           post_emb_dict, user_emb_dict, news_emb_dict):
        ...
        if node_type == "news":
            ...
            linear_input_title = linear_input_title.view(
                linear_input_title.shape[0], 1, linear_input_title.shape[1]
            )
            ...
            # ========== 强 MDF 消融（核心） ==========
            if self.training:
                idx = torch.randperm(mean_pooling.size(0))
                mean_pooling = mean_pooling[idx]

            noise = torch.randn_like(mean_pooling) * 0.5
            mean_pooling = mean_pooling + noise

            mean_pooling = torch.nn.functional.dropout(
                mean_pooling, p=0.6, training=self.training
            )
            # =======================================
        ...
        if node_type == "post":
            ...
            for i in neighbor_id:
                if ("post", i) not in self.content_dict:
                    input_a.append(post_emb_dict[i][1])  # text
                    input_b.append(post_emb_dict[i][0])  # other
            ...
            linear_input_text = linear_input_text.view(
                linear_input_text.shape[0], 1, linear_input_text.shape[1]
            )
...
def fold_train_test(data_real, data_fake, test_size, PATH):
    ...
    # ========== Step 1: 过滤无效节点 ==========
    def is_valid(node):
        return node.node_id in neighbor_dict[2] and node.node_id in n_add_time_dict

    original_train_len = len(train_val)
    train_val = [node for node in train_val if is_valid(node)]
    filtered_train_len = len(train_val)

    original_test_len = len(test_set)
    test_set = [node for node in test_set if is_valid(node)]
    filtered_test_len = len(test_set)

    print("\n[INFO] Filtering summary:")
    print(f"Train set: Kept {filtered_train_len} / {original_train_len} ({filtered_train_len/original_train_len:.2%})")
    print(f"Test set : Kept {filtered_test_len} / {original_test_len} ({filtered_test_len/original_test_len:.2%})")
    ...
    with open(PATH + 'fold_train_index.pkl', 'wb') as f:
        pickle.dump(train_index, f)
    with open(PATH + 'fold_val_index.pkl', 'wb') as f:
        pickle.dump(val_index, f)
    with open(PATH + 'fold_train_val.pkl', 'wb') as f:
        pickle.dump(train_val, f)
    with open(PATH + 'fold_test_set.pkl', 'wb') as f:
...
def load_fold_train_test(PATH):
    import pickle
    with open(PATH + 'fold_train_index.pkl', 'rb') as f:
        train_index = pickle.load(f)
    with open(PATH + 'fold_val_index.pkl', 'rb') as f:
        val_index = pickle.load(f)
    with open(PATH + 'fold_train_val.pkl', 'rb') as f:
        train_val = pickle.load(f)
    with open(PATH + 'fold_test_set.pkl', 'rb') as f:
...
five_res = []
base_PATH ='/home/zhangrq/DPSG/models/ablation/gossipcop_minus_TG/'
if not os.path.exists(base_PATH):
    os.makedirs(base_PATH)
f_path = base_PATH + 'result-time.txt'
...
for repeat in range(5):
    ...
    with open(f_path, "a") as f:
        f.write("Repeat:%s\n" % (repeat + 1))
    train_index, val_index, train_val, test_set = fold_train_test(news_nodes_real, news_nodes_fake, 0.1, PATH)
    ...
    for fold in range(num_folds):
        ...
        for epoch in range(start_epoch,num_epoch):
            ...
            net.train()
            batch_count = 0  # 用来统计有效 batch 
            skipped_nodes = []
            for i in range(len(train_set)):
                optimizer.zero_grad()
                node_id = train_set[i].node_id
                # 构造 all_t 前保险判断，防止意外 key 错误
                if node_id not in neighbor_dict[2] or node_id not in n_add_time_dict:
                    continue  # 跳过
                all_t = []
                ...
                # ✅ 在有效 batch 的第一个样本时，初始化 loss
                if batch_count % batch_size == 0:
                    loss = torch.tensor(0.0, device=device, requires_grad=True)
                    loss_dis = torch.tensor(0.0, device=device, requires_grad=True)
                loss = loss + cur_loss
                loss_dis = loss_dis + cur_loss_dis
                batch_count += 1  # 只对有效样本计数
    ...
    for k in range(len(test_set)):
        ...
        if test_set[k].label == 1:
            ...
            if output / (1 - output) < ratio:
                fake_true += 1
    print(f"[SUMMARY] Test skipped nodes: {test_skipped} / {len(test_set)} ({test_skipped / len(test_set):.2%})")
