...
class DPSG(nn.Module):
    def __init__(self, input_dim, n_hidden_dim, u_hidden_dim, p_hidden_dim, out_embed_d, outemb_d=1,
                 content_dict={}, attn_heads=8, d_model=104, self_attn_heads=8, enc_layers=1, dec_layers=1,max_nodes=10000,ablation_egse=False):
        ...
        #新内容
        self.ablation_egse = ablation_egse
    ...
    def transformer_agg(self, het_node, neighbor_order_n_p_u, neighbor_order_n, all_t0, npu=15, n=5):
        ...
        if not self.ablation_egse:
            ...
            gx, graph_feature = self.gat_relation.forward(
            ...
            # ===== Ablation: -EGSE =====
            # 不使用图结构增强，构造“空图特征”
            gx = node_embedding.clone()
            graph_feature = node_embedding_n.clone()
        ...
        self_att_c_all = self.mh_attention1(encoder_emb_input.view(bsz_all, -1, self.hidden_dim),
                                            ...
                                            encoder_emb_input.view(bsz_all, -1, self.hidden_dim))  # [1+n_nei_num+p+u,1,H]
        ...
        #新内容
        # ===== Transformer 输入构造 =====
        encoder_emb_input += self.type_encoder(encoder_type_input).view(
            encoder_emb_input.shape[0], 1, encoder_emb_input.shape[2]
        )
        encoder_emb_input += self.pos_encoder(encoder_emb_input)

        decoder_emb_input += self.pos_encoder(decoder_emb_input)
        decoder_emb_input += self.type_encoder(decoder_type_input).view(
            decoder_emb_input.shape[0], 1, decoder_emb_input.shape[2]
        )

        # ===== EGSE 注入（或消融）=====
        if not self.ablation_egse:
            encoder_emb_input += align_g_all.view(
                encoder_emb_input.shape[0], 1, encoder_emb_input.shape[2]
            )
            decoder_emb_input += align_g.view(
                decoder_emb_input.shape[0], 1, decoder_emb_input.shape[2]
            )
...
five_res = []
base_PATH = '/home/zhangrq/DPSG/models/ablation/pheme_minus_RE/'
...
for repeat in range(5):
    ...
    for fold in range(start_fold,num_folds):
        ...
        net = DPSG(input_dim=[768,512, 6], n_hidden_dim=hidden_dim, u_hidden_dim=hidden_dim, p_hidden_dim=hidden_dim,
                             ...
                             self_attn_heads=8, enc_layers=1, dec_layers=1,max_nodes=10000,ablation_egse=True)
        net.to(device)
        net.init_weights()
    ...
    net = DPSG(input_dim=[768,512, 6], n_hidden_dim=hidden_dim, u_hidden_dim=hidden_dim, p_hidden_dim=hidden_dim,
                         ...
                         enc_layers=1, dec_layers=1,max_nodes=10000,ablation_egse=True)
    net.to(device)
    net.init_weights()
    optimizer = optim.SGD(net.parameters(), lr=lr)
