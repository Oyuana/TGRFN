...
neighbor_dict = neighbor_loader('/home/zhangrq/DPSG/data/rwr_results/fnn_politifact_n5_p5_u100/n_neighbors.txt')

with open("/home/zhangrq/DPSG/data/rwr_results/fnn_politifact_n5_p5_u100/original_adj",'r',encoding='utf-8') as f:
    adj_list=json.load(f)

def data_loader(pathway='data/processed_data/FakeNewsNet/OldPolitiFact/batch/normalized_news_nodes/', node_type="post"):
    if node_type == "news":
        news_node = []
        news_id = []
        news_label = []
...
post_nodes = data_loader(
    pathway='/home/zhangrq/DPSG/data/processed_data/FakeNewsNet/PolitiFact/fnn_politifact_n5_p5_u100/normalized_post_nodes/',
    node_type="post")
user_nodes = data_loader(
    pathway='/home/zhangrq/DPSG/data/processed_data/FakeNewsNet/PolitiFact/fnn_politifact_n5_p5_u100/normalized_user_nodes/',
    node_type="user")
news_nodes = data_loader(
    pathway='/home/zhangrq/DPSG/data/processed_data/FakeNewsNet/PolitiFact/fnn_politifact_n5_p5_u100/normalized_news_nodes/',
    node_type="news")
news_emb_dict = {}
post_emb_dict = {}
user_emb_dict = {}
...
class Signed_GAT(nn.Module):
    ...
    def forward(self,node_embedding,cosmatrix,original_adj, X_tid):
        ...
        self.attentions = [GraphAttentionLayer(embedding_dim, embedding_dim, dropout=self.dropout, alpha=self.alpha, concat=True) for _
            in range(self.nb_heads)]
...
class DPSG(nn.Module):
    def __init__(self, input_dim,n_hidden_dim,u_hidden_dim,p_hidden_dim,
        ...
        self.transformer = nn.Transformer(d_model=d_model, nhead=attn_heads, num_encoder_layers=6, num_decoder_layers=6,
                                          dim_feedforward=512,dropout=0.1, activation='relu', custom_encoder=None, custom_decoder=None)
        ...
        self.encoder_lstm = nn.LSTM(self.d_model, self.d_model, 1, batch_first=True)
    ...
    def transformer_agg(self, het_node, neighbor_order_n_p_u, neighbor_order_n,all_t0, npu=200,n=5):
        ...
        self_att_g = self.mh_attention1(graph_feature.view(bsz, -1, self.hidden_dim), graph_feature.view(bsz, -1, self.hidden_dim), \
        ...
        decoder_emb_input = decoder_emb_input.view(decoder_emb_input.shape[0], 1,
                                                   decoder_emb_input.shape[1])
    ...
    def forward(self, x, neighbor_order_n_p_u, neighbor_order_n,all_t):
        x,dist = self.transformer_agg(x, neighbor_order_n_p_u, neighbor_order_n,all_t, npu=self.npu)
...
five_res = []
#base_PATH = '../pre-trained/PolitiFact/'
base_PATH = '/home/zhangrq/DPSG/models/ablation/politifact_minus_CA/'
if not os.path.exists(base_PATH):
    os.makedirs(base_PATH)
f_path = base_PATH + 'result-atten.txt'
for repeat in range(5):
    ...
    with open(f_path, "a") as f:
        ...
        f.write("PATH:%s\n" % (PATH))
        print("Written lr, batch_size, and PATH to file")  # Debug statement
    ...
    for fold in range(num_folds):
        ...
        test_loss = 0.0
        best_epoch = 0
        net = DPSG(input_dim=[768, 512, 3, 9],
        ...
        for epoch in range(num_epoch):
            ...
            for i in range(len(train_set)):
                ...
                cur_loss_dis = torch.zeros(1).to(device)
            ...
            for j in range(len(val_set)):
                ...
                output,dist_og = net(val_set[j], neighbor_dict[0][val_set[j].node_id], neighbor_dict[1][val_set[j].node_id],all_t)
                out = output.item()+epsilon
                ...
                vloss_dis = torch.zeros(1).to(device)

            val_acc = v / len(val_set)

            real_precision = real_true / (real_true + fake_count - fake_true+epsilon)
            fake_precision = fake_true / (fake_true + real_count - real_true+epsilon)
            real_recall = real_true / (real_count+epsilon)
            fake_recall = fake_true / (fake_count+epsilon)
            real_f1 = 2 * real_precision * real_recall / (real_precision + real_recall+epsilon)
            fake_f1 = 2 * fake_precision * fake_recall / (fake_precision + fake_recall+epsilon)
            ...
            print("time for a epoch: ", t_current_end - t_current_start, )
            with open(f_path, "a") as f:
    ...
    for k in range(len(test_set)):
        ...
        for fold in range(num_folds):
            ...
            # ===== w/o CA: do not use alignment loss =====
            tloss_dis = torch.zeros(1).to(device)
