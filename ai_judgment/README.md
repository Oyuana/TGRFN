...
`inference.py::InferenceEngine.load_data_bundle()` 现已对接
`data_adapter.build_data_bundle()`——它**无副作用**地复用 `GossipCop.py` 的
`neighbor_loader` / `data_loader` 落盘格式，从 7 个路径环境变量装配 `DataStore`
与各 `news_id` 的邻居顺序、时间戳，并自动从 news label 统计 `ratio`。

部署时**通常无需改代码**，只要配好环境变量指向真实数据；任一路径缺失即视为
"数据未挂载"，API 以 503、`/health` 以 `degraded` 明确报错，不会静默给假结果。
仅当队员的落盘格式与 GossipCop.py 不一致时，才需改 `data_adapter.py` 的解析函数。

> **拿到真实数据集后的完整操作步骤见 [`RUN_GUIDE.md`](./RUN_GUIDE.md)。**
