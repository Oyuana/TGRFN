...
#post_nodes_fnn.py: 每条 tweet 的特征 + RWR 采样邻居
#user_nodes_fnn.py: 每个用户的 profile + 行为特征
#news_nodes_fnn.py: 每条新闻的文本嵌入 + 图结构关系

#基于 FakeNewsNet 数据集（如 gossipcop）构建三类节点（news、post、user）的图神经网络输入数据（节点特征 + 邻居信息）并并行处理保存
