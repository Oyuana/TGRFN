...
#对gossipcop 数据集中的新闻标题、正文、推文内容和用户描述，进行 批量嵌入提取（文本向量化），并保存为 .txt 文件。
...
#新闻采用 T5 模型（适合长文本摘要）
#推文/用户描述采用 BERTweet 模型（专为社交媒体设计）
#多进程并行加速嵌入处理
configs = {
    'news' : {
...
device = 'cpu'
#使用指定预训练语言模型对一批文本进行嵌入（在子进程中并行运行）
...
#加载推文文本（从 tweet 和 retweet 文件中），提取嵌入
def process_tweets():
