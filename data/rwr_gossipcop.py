...
#为每个节点构建基于随机游走的邻居集合（RWR邻接表），用于后续动态图建模。实现了在传播图（异构图）中，从每个节点出发进行随机游走（带重启），以收集其最有代表性的邻居，并输出到文件中

#从邻居列表中重新计算各类型节点的“被涉及集合”（即哪些节点有参与）
...
def random_walk_with_restart(node_types,min_neigh,max_uniq_neigh,edges_to_enforce):
    ...
    involved = {t : set() for t in node_types}
    manager = Manager()
#	构造原始无向图的邻接表 adj_list
    def add_adjacent(m, n):
        if m not in adj_list.keys():
            adj_list[m] = []
        adj_list[m].append(n)
#多进程汇总局部邻接结果到主邻接字典 nei_list
...
if __name__ == "__main__":
    ...
    for dataset in ['gossipcop']:
        if dataset in ['gossipcop']:
        ...
        if typed_rwr:
            ...
            configuration_tag = prefix + '_'.join([f'{k}{max_uniq_neigh[k]}' for k in node_types])+'_v1'
        elif no_rwr:
