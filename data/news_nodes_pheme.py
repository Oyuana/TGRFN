...
#为 PHEME 数据集 中的 新闻（news）节点、帖子（post）节点、用户（user）节点 构建标准化后的多模态特征文件

def create_news_nodes(folder_name):
#读取新闻节点的文本特征、邻居信息、真假标签；

# 读取每条新闻的邻居节点（post、user、news）；

# 提取并标准化每类节点的嵌入特征（news text, user description）；
...
#-------------------------------news-----------------------------------------------------
#（每5000条一个 batch）：
# n 28020191104153001 0      id和标签
# 0.123 0.456 ...          ← 新闻文本嵌入向量
# n123 n456                ← 相邻新闻节点
# p234 p567                ← 相邻post节点
# u777 u888                ← 相邻user节点
...
# 0.321 0.111 ...      即文本嵌入向量
    ...
    # -------------------user---------------------------
# u 29120191106113007
# 1 0 0 1 0 1 ...             ← 用户属性（one-hot）
# 0.123 0.456 ...             ← 用户描述嵌入（如 Roberta）
