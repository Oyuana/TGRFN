...
#为 FakeNewsNet 数据集中的推文节点（post）提取数值特征（转发数、点赞数、是否为引用推文）的工具

#最后的post特征为p12345678: 34 100 0，34 100 0：分别是转发数、点赞数、是否为引用
    ...
    for post_type in ['fake/', 'real/']:
        ...
        for file in tqdm(news_list):
            ...
            for tweet_file in tweets_list:
                ...
                num_retweet = str(data['retweet_count'])#转发数
                    
                num_favorite = str(data['favorite_count'])#点赞数
                is_quote_status = str(data['is_quote_status']-0)#是否为引用
