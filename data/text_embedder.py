...
#定义并测试了一个统一封装的文本嵌入模块 TextEmbedder，用于将输入文本（如新闻、推文等）转换为深度学习模型可以使用的 固定维度向量表示，并支持多种主流预训练模型
class TextEmbedder(torch.nn.Module):
    #初始化函数，根据所选模型加载相应的 tokenizer 与 model
    def __init__(self, max_seq_len : int, model_name: str, model_path: Optional[str] = '', device: Optional[str] = 'cpu'):
        ...
        self.max_seq_len = max_seq_len #控制最大 token 数量（截断 + padding）
...
#将一批文本转换为张量（tensor）嵌入表示
    def forward(self, text_list: List[str], return_tokens: Optional[bool] = False) -> Tuple[torch.Tensor, List[List[str]]]:
        #text_list: 文本列表（List[str]） return_tokens: 是否返回分词后的结果（调试用） outputs: 形状为 (B, L, D) 的张量，B=文本数量, L=最大长度, D=嵌入维度
